...
% Standard fonts (Computer Modern - LaTeX default)
...
% Better justification / character protrusion
\usepackage[protrusion=true,expansion=false]{microtype}

% Hyperlinks - load last, enables clickable refs and citations
\usepackage[colorlinks=true,linkcolor=blue,citecolor=blue,urlcolor=blue]{hyperref}

% Allow a bit more stretch for tricky paragraphs (long \texttt strings)
...
% TITLE
%=============================================================================
% AUTHOR PRESENTATION OPTIONS
% Uncomment ONE of the following options:
%=============================================================================

% OPTION 2: Author with AI Assistant credit line
\newcommand{\authorblock}{%
\author{Jo\~ao Galego$^{1}$\\[0.5em]
\small\itshape $^1$Independent Researcher\\[1.2em]
\normalsize\textsc{AI Research Assistant}: Claude Opus 4.7 (Anthropic)}
}

%=============================================================================

\title{No CUDA, No Problem:\\[0.3em]
       Lattice QCD on AWS Trainium and Inferentia via PyTorch--XLA}
...
across all on-chip NeuronCores. On a two-core Inf2 (\texttt{inf2.xlarge})
in BF16 we obtain up to $13.0\times$ throughput over a tuned FP32 CPU
baseline for the Wilson Dslash on $V=4{\times}4{\times}4{\times}4$ ($B{=}8$
RHS per core), and establish that throughput reaches CPU parity at
$V=16{\times}8{\times}8{\times}8$ and falls below beyond that, all with no
custom kernel code. We emphasise that these speedups are achieved on
sub-production volumes; sustained throughput remains $\le 7$\,GFLOP/s,
well below both the NeuronCore-v2 BF16 peak and published QUDA
numbers for Wilson Dslash on contemporary GPUs. The work is therefore
best read as a feasibility / lessons-learned study of the AoT-compiled,
static-graph programming model rather than a competitive
implementation. Source code is available at
\href{https://github.com/JGalego/lqcd-neuron}{github.com/JGalego/lqcd-neuron}
under the Apache 2.0 license. All calculations, numerical analysis, and
manuscript preparation were performed by Claude, an AI assistant developed
by Anthropic, working under physicist supervision.
\end{abstract}
...
$\gamma_\mu$ acting on the four-component spinor. We use the
unhopped (mass-form) normalisation; the equivalent $\kappa$-form,
$\kappa = 1/(2(m+4))$, is recovered by rescaling $\psi$. Anti-periodic
temporal boundary conditions are imposed by multiplying the temporal
gauge links on the boundary timeslice by $-1$, after which all shifts
can be implemented as periodic \texttt{torch.roll}s.
...
The Euclidean Dirac matrices are stored in the DeGrand--Rossi
...
For Conjugate Gradient~\cite{HestenesStiefel} (and analogously
BiCGStab~\cite{vanderVorst}), schematically:
\begin{lstlisting}
...
agrees with the reference \texttt{forward()} to a relative error
$\lesssim 10^{-7}$ in FP32, and continues to satisfy
the existing $\gamma_5$-Hermiticity, adjoint, and normal-operator
positivity tests.

\paragraph*{SRAM budget guard.}
The eight $K^{\mathrm{fwd/bwd}}$ buffers have per-lattice footprint
$8V\!\cdot\!144\!\cdot\!8$ bytes (complex entries stored as a pair of
float32 reals).  This already exceeds the 14.4\,MiB NeuronCore on-chip
SRAM at $V=16{\times}8{\times}8{\times}8$ (72\,MiB) and reaches
576\,MiB at $V=16{\times}16{\times}16{\times}16$.  The compiler therefore checks the fused
footprint against a configurable SRAM threshold before tracing; if the
budget is exceeded it issues a warning and falls back to the unfused
baked-gauge path, preserving correctness at reduced MXU efficiency.
The threshold can be overridden via
\texttt{NeuronCompiler(sram\_threshold\_bytes=N)} or suppressed with
\texttt{fused=False}.
...
\texttt{compile\_dslash\_batched(}$D$, $\mathrm{shape}$,
\texttt{batch\_size=}$B$, \texttt{gauge\_field=}$U$\texttt{)},
which traces the fused operator on
...
all detected cores via \texttt{torch\_neuronx.}\allowbreak\texttt{DataParallel} and
...
Multicore column relative to CPU; entries at or below $1\times$ indicate
the optimised Neuron path has not yet overcome the dispatch and
bandwidth bottleneck at that volume.  GFLOP/s and GB/s are derived
from the Multicore column (1320 flop/site~\cite{Babich:2011np};
streaming spinor in${+}$out, BF16; gauge baked, no PCIe traffic for
$U$). For context, NeuronCore-v2 BF16 peak is
$\sim\!95$\,TFLOP/s per core and HBM bandwidth is
$\sim\!400$\,GB/s per chip, so the sustained figures correspond to
$\lesssim 10^{-4}$ of compute peak and $\lesssim 10^{-3}$ of memory peak.
$\dagger$~Fused kernels exceed the
14.4\,MiB NeuronCore SRAM budget at this volume; the compiler
automatically falls back to the unfused baked-gauge path.}
\label{tab:bench}
\centering
\footnotesize
...
\midrule
$4{\times}4{\times}4{\times}4$           &  804.6 & 1358.0 &  8521.5 & \textbf{10431.3} & $13.0\times$ & 3.52 & 0.26 \\
$8{\times}4{\times}4{\times}4$           &  769.5 &  785.1 &  4786.3 &  \textbf{7519.1} &  $9.8\times$ & 5.08 & 0.37 \\
$8{\times}8{\times}4{\times}4$           &  518.1 &  370.1 &  2511.1 &  \textbf{4229.5} &  $8.2\times$ & 5.72 & 0.42 \\
$8{\times}8{\times}8{\times}4$           &  384.0 &  207.6 &  1390.1 &  \textbf{2474.2} &  $6.4\times$ & 6.69 & 0.49 \\
$V=16{\times}8{\times}8{\times}8$          &  229.2 &   37.2 &   253.8 &            220.1 & $0.96\times^\dagger$ & 2.38 & 0.17 \\
$16{\times}16{\times}16{\times}16$       &   59.6 &    1.2 &    24.4 &             15.8 & $0.3\times^\dagger$ & 1.36 & 0.10 \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:bench} reports the throughput of the optimised
\textsc{LQCD-Neuron} Wilson Dslash on an
\texttt{inf2.8xlarge},
...
volumes up to $V=8{\times}8{\times}8{\times}4$, but reaches only CPU parity at
$V=16{\times}8{\times}8{\times}8$ ($1.0\times$ with Multicore)$^\dagger$ and falls
clearly below at $V=16{\times}16{\times}16{\times}16$ ($0.3\times$ with Multicore)$^\dagger$;
at $V=16{\times}8{\times}8{\times}8$ Multicore reaches $0.96\times$\,CPU,
which we describe as parity to one significant figure.
At both large volumes the fused $12{\times}12$ kernel buffers alone
exceed the 14.4\,MiB NeuronCore on-chip SRAM (72\,MiB at
$16{\times}8{\times}8{\times}8$ and 576\,MiB at $16{\times}16{\times}16{\times}16$), so the
compiler automatically falls back to the unfused baked-gauge path,
eliminating the MXU-utilisation benefit of the fused kernels.
Third, the two-core \texttt{DataParallel} (Multicore) configuration
delivers $6.4$--$13.0\times$ over CPU from $V=4^4$ through
$V=8{\times}8{\times}8{\times}4$. The breakeven volume - beyond which the
CPU baseline is faster even with all optimisations - is now at
$V=16{\times}8{\times}8{\times}8$ (CPU parity, $1.0\times$); the fused-kernel
SRAM limit is the primary obstacle to recovering the $2\times$--$3\times$
advantage that would otherwise be expected at these volumes, and
bridging it (via kernel tiling or NeuronCore SRAM bucketing) is the
primary motivation for the work outlined in Sec.~\ref{sec:outlook}.
...
The most awkward limitation is the static-shape requirement:
each tuple
$(\mathrm{class},\mathrm{shape},\mathrm{dtype},B)$
compiles to a distinct \NEFF. For a fixed production volume this is amortised once,
...
custom kernel code reaches up to $13.0\times$ over a tuned CPU
baseline on $V=4{\times}4{\times}4{\times}4$ in BF16, with CPU parity
at $V=16{\times}8{\times}8{\times}8$ and a sustained throughput that
remains far below both NeuronCore peak and published QUDA numbers.
Closing that gap will require even-odd preconditioning, SU(3)
reconstruction to relieve memory pressure, and an SRAM-aware tiled
fused kernel; the present paper is best read as a feasibility report
establishing the programming model rather than a competitive
implementation.
...
J.G.\ conceived and directed the project, defined the scope and target
hardware, guided the AI assistant throughout development, validated the
benchmark results on AWS Inferentia (Inf2) instances, and reviewed all
code and prose for scientific and engineering correctness. Claude, an AI
research assistant developed by Anthropic, performed the implementation
work-including the \texttt{torch.nn.Module} kernel design, the four
graph-level optimisations (gauge baking, fused $12{\times}12$
spin--colour kernels, multi-RHS batching, and multi-core
\texttt{DataParallel} sharding), the unit-test suite, the benchmark
harness, the numerical analysis, and the manuscript preparation. The
work was conducted using Claude Code, Anthropic's agentic coding tool.
J.G.\ is fully responsible for the scientific and technical content
...
\bibitem{vanderVorst}
H.~A.~van der Vorst,
\emph{Bi-CGSTAB: A fast and smoothly converging variant of Bi-CG
