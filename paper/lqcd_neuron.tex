...
in BF16 we obtain up to $10.9\times$ throughput over a tuned FP32 CPU
baseline for the Wilson Dslash on $V=4{\times}4{\times}4{\times}4$, and demonstrate that the
optimised path remains competitive at $V=8{\times}8{\times}8{\times}4$ with no
...
below CPU at modest volumes to up to $10.9\times$ above it on a single
NeuronCore.
...
\begin{table}[t]
\caption{Wilson Dslash sustained throughput on a single Inf2
NeuronCore-v2 (BF16, gauge baked) versus a tuned CPU baseline (FP32),
in Dslash applications per second over 50 iterations after a
5-iteration warm-up. ``8-RHS'' uses
\texttt{compile\_dslash\_batched} with $B{=}8$. Speed-up is the
best Neuron column relative to CPU.}
\label{tab:bench}
\centering
\footnotesize
...
\begin{tabular}{lrrrr}
\toprule
Lattice & CPU & N. 1-RHS & N. 8-RHS & Speed-up \\
\midrule
$4{\times}4{\times}4{\times}4$               &  818 & 1419 & \textbf{8957} & $10.9\times$ \\
$8{\times}4{\times}4{\times}4$               &  767 &  794 & \textbf{4918} &  $6.4\times$ \\
$8{\times}8{\times}4{\times}4$               &  646 &  372 & \textbf{2481} &  $3.8\times$ \\
$8{\times}8{\times}8{\times}4$               &  449 &  208 & \textbf{1379} &  $3.1\times$ \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:bench} reports the throughput of the optimised
\textsc{LQCD-Neuron} Wilson Dslash on a single Inf2 NeuronCore-v2,
...
faster than CPU at the smallest volume; at $V=8{\times}8{\times}8{\times}4$ it is
slower by a factor of two. This confirms our hypothesis that the
naive Dslash graph is dispatch- and shape-limited: the work per call
is too small to amortise the $\sim\!1\,$ms NeuronCore launch cost,
and the underlying $3{\times}3$ / $4{\times}4$ einsums leave the MXU
idle. Second, multi-RHS batching alone closes the gap and overtakes
CPU by $3$--$11\times$ across the lattice range, with the largest
relative gain at small $V$ where dispatch dominates. Third, the
relative speed-up shrinks with volume because the CPU baseline scales
nearly linearly while the on-chip BF16 throughput is bounded by
NeuronCore-resident memory bandwidth at this size; the multi-core
\texttt{DataParallel} path is designed to recover scaling and is
described above.
